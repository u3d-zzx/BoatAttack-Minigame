...
{
	float rawD = SampleSceneDepth(uvs);//SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_ScreenTextures_linear_clamp, uvs);
...
{
	//posWS.xz * 0.002 + 0.5
    return (1 - SAMPLE_TEXTURE2D_LOD(_WaterDepthMap, sampler_WaterDepthMap_linear_clamp, float2(posWS.x,0- posWS.z)*0.002 + 0.5, 1).r) * (_MaxDepth + _VeraslWater_DepthCamParams.x) - _VeraslWater_DepthCamParams.x;
}
...
half4 WaterFragment(WaterVertexOutput IN) : SV_Target
{
	...
	half3 sss = GI + mainLight.color * shadow;
