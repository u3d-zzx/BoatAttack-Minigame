...
namespace BoatAttack
{
    ...
    public class BoatDataDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var color1Rect = new Rect(position.position + new Vector2(humanRect.width, singleHeight + singleSpace),
                halfSize * new Vector2(0.5f, 1f));
            var color2Rect =
                new Rect(
                    position.position + new Vector2(humanRect.width + color1Rect.width, singleHeight + singleSpace),
                    halfSize * new Vector2(0.5f, 1f));
            ...
            var prefabProp = property.FindPropertyRelative(nameof(BoatData.boatABIndex));
            ...
            EditorGUI.PropertyField(color1Rect,
                property.FindPropertyRelative(nameof(BoatData.livery))
                    .FindPropertyRelative(nameof(BoatLivery.primaryColor)));
            EditorGUI.PropertyField(color2Rect,
                property.FindPropertyRelative(nameof(BoatData.livery))
                    .FindPropertyRelative(nameof(BoatLivery.trimColor)));
